...
public class JDKAsyncHttpProvider implements AsyncHttpProvider {
	...
	public JDKAsyncHttpProvider(final AsyncHttpClientConfig config) {
		...
		this.config = config;
		final AsyncHttpProviderConfig<?, ?> providerConfig = config
				.getAsyncHttpProviderConfig();
		if (providerConfig != null
				&& JDKAsyncHttpProviderConfig.class
						.isAssignableFrom(providerConfig.getClass())) {
	...
	public <T> ListenableFuture<T> execute(final Request request,
			final AsyncHandler<T> handler, final ListenableFuture<?> future)
			throws IOException {
		if (isClose.get()) {
		...
		final HttpURLConnection urlConnection = createUrlConnection(request);

		final PerRequestConfig conf = request.getPerRequestConfig();
		final int requestTimeout = (conf != null && conf
				.getRequestTimeoutInMs() != 0) ? conf.getRequestTimeoutInMs()
		...
		final JDKFuture f = delegate == null ? new JDKFuture<T>(handler,
	...
	private HttpURLConnection createUrlConnection(final Request request)
			throws IOException {
		final ProxyServer proxyServer = request.getProxyServer() != null ? request
				.getProxyServer() : config.getProxyServer();
		final Realm realm = request.getRealm() != null ? request.getRealm()
		...
		if (proxy == null) {
			urlConnection = (HttpURLConnection) AsyncHttpProviderUtils
					.createUri(request.getUrl()).toURL()
					.openConnection(Proxy.NO_PROXY);
		} else {
			urlConnection = (HttpURLConnection) AsyncHttpProviderUtils
					.createUri(request.getUrl()).toURL().openConnection(proxy);
		}
	...
	private final class AsyncHttpUrlConnection<T> implements Callable<T> {
		...
		public AsyncHttpUrlConnection(final HttpURLConnection urlConnection,
		...
		public T call() throws Exception {
			...
			try {
				...
				} catch (final IllegalArgumentException u) {
				...
				if (TransferCompletionHandler.class
						.isAssignableFrom(asyncHandler.getClass())) {
				...
				final ResponseStatus status = new ResponseStatus(uri,
						urlConnection, JDKAsyncHttpProvider.this);
				...
				final boolean redirectEnabled = (request.isRedirectEnabled() || config
						.isRedirectEnabled());
				if (redirectEnabled && (statusCode == 302 || statusCode == 301)) {

					if (currentRedirectCount++ < config.getMaxRedirects()) {
						final String location = urlConnection
								.getHeaderField("Location");
						final URI redirUri = AsyncHttpProviderUtils
								.getRedirectUri(uri, location);
				...
				if (statusCode == 401 && !isAuth.getAndSet(true)
						&& realm != null) {
					final String wwwAuth = urlConnection
				...
				if (state == AsyncHandler.STATE.CONTINUE) {
					...
					final String contentEncoding = urlConnection
							.getHeaderField("Content-Encoding");
					final boolean isGZipped = contentEncoding == null ? false
				...
				try {
					final T t = asyncHandler.onCompleted();
					future.content(t);
					future.done(null);
					return t;
		...
		private void configure(final URI uri,
				final HttpURLConnection urlConnection, final Request request)
				throws IOException, AuthenticationException {

			final PerRequestConfig conf = request.getPerRequestConfig();
			final int requestTimeout = (conf != null && conf
					.getRequestTimeoutInMs() != 0) ? conf
			...
			final String ka = config.getAllowPoolingConnection() ? "keep-alive"
					: "close";
			...
			if ("POST".equals(reqType) || "PUT".equals(reqType)) {
				...
				if (cachedBytes != null) {
					...
					final byte[] b = request.getStringData().getBytes(
							bodyCharset);
					urlConnection.setRequestProperty("Content-Length",
	...
	private Proxy configureProxyAndAuth(final ProxyServer proxyServer,
		...
		if (proxyServer != null) {
			...
			final String proxyHost = proxyServer.getHost()
					.startsWith("http://") ? proxyServer.getHost().substring(
					"http://".length()) : proxyServer.getHost();

			final SocketAddress addr = new InetSocketAddress(proxyHost,
					proxyServer.getPort());
	...
	private InputStream getInputStream(final HttpURLConnection urlConnection)
			throws IOException {
